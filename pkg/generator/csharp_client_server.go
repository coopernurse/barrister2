...
func (p *CSharpClientServer) Generate(idl *parser.IDL, fs *flag.FlagSet) error {
	...
	// Generate Server.cs
	serverCode := generateServerCs(idl, structMap, enumMap, namespaceMap)
	...
	if generateTestServer {
		...
		testClientCode := generateTestClientCs(idl, structMap, enumMap)
...
func generateServerCs(idl *parser.IDL, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, namespaceMap map[string]*NamespaceTypes) string {
	var sb strings.Builder
...
func writeBarristerServerCs(sb *strings.Builder, idl *parser.IDL) {
	...
	sb.WriteString("        }\n")
	sb.WriteString("    }\n\n")
	sb.WriteString("    private void ConvertEnumIntsToStrings(Dictionary<string, object?> dict, Dictionary<string, Dictionary<string, object>> allStructs, Dictionary<string, Dictionary<string, object>> allEnums)\n")
	sb.WriteString("    {\n")
	...
	sb.WriteString("            {\n")
	sb.WriteString("                ConvertEnumIntsToStrings(nestedDict, allStructs, allEnums);\n")
	sb.WriteString("            }\n")
	...
	sb.WriteString("                    {\n")
	sb.WriteString("                        ConvertEnumIntsToStrings(listDict, allStructs, allEnums);\n")
	sb.WriteString("                    }\n")
	sb.WriteString("                }\n")
	sb.WriteString("            }\n")
	sb.WriteString("            else if (value is int intVal)\n")
	sb.WriteString("            {\n")
	sb.WriteString("                // Check if this integer might be an enum value by trying all enum definitions\n")
	sb.WriteString("                foreach (var enumKvp in allEnums)\n")
	sb.WriteString("                {\n")
	sb.WriteString("                    var enumDef = enumKvp.Value;\n")
...
func writeHandleSingleRequestCs(sb *strings.Builder, idl *parser.IDL) {
	...
	sb.WriteString("            catch (Exception e)\n")
	sb.WriteString("            {\n")
	sb.WriteString("                _logger?.LogError(e, \"Failed to load IDL JSON\");\n")
...
func writeMethodLookupAndInvokeCs(sb *strings.Builder, idl *parser.IDL) {
	...
	sb.WriteString("                        var structResultJson = JsonSerializer.Serialize(result);\n")
	...
	sb.WriteString("                        {\n")
	sb.WriteString("                            ConvertEnumIntsToStrings(structDict, IdlData.ALL_STRUCTS, IdlData.ALL_ENUMS);\n")
	sb.WriteString("                        }\n")
	sb.WriteString("                    }\n")
	sb.WriteString("                    else\n")
	sb.WriteString("                    {\n")
...
func generateTestServerCs(idl *parser.IDL, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum) string {
	var sb strings.Builder
...
func generateTestClientCs(idl *parser.IDL, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum) string {
	var sb strings.Builder
