...
import com.example.myapp.checkout.CatalogService;
import com.example.myapp.checkout.CartService;
import com.example.myapp.checkout.OrderService;
...
public class Server {
    ...
    private Map<String, Object> handleJsonRpcRequest(Map<String, Object> request) {
        ...
        if ("barrister-idl".equals(method)) {
            // Return IDL definition - read from idl.json in resources
            try {
                InputStream is = Server.class.getResourceAsStream("/idl.json");
                if (is == null) {
                    return Map.of(
                        "jsonrpc", "2.0",
                        "error", Map.of(
                            "code", -32603,
                            "message", "Failed to load IDL: /idl.json not found in classpath"
                        ),
                        "id", id
                    );
                }
                String idlJson = new String(is.readAllBytes());
